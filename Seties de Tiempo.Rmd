...
Una notación muy útil para considerar este tipo de modelos se basa en el uso del operador de retardo $B$. El modelo $AR(p)$ es
...
$\textbf{Definición 2.1}$ Se dice que una serie temporal $Y_{t}$ admite una representación de medias móviles (MA) de orden $q$ , y se denota por $MA(q)$ , si es susceptible de ser modelizada a través de la ecuación:
\begin{align}
...
Donde $Y_{t}$ es una variable aleatoria concebida como realizaciones de un proceso estocástico en los momentos del tiempo $t$ , que se caracterizan por $\mathbb{E}(Y_{t})=\mathbb{E}(Y_{t−1})=\mathbb{E}(Y_{t−2})=\cdots$, $\theta_{1},...,\theta_{q}$ son parámetros del modelo que, junto con la varianza deben ser estimados, y $\varepsilon_{t}\sim RB(0,\sigma_{\varepsilon}^{2})$.
